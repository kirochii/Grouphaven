...
export default function UserProfile() {
  ...
  const fetchUser = async () => {
    ...
    if (error) {
      ...
      setLocation(getLocation(data.city));
    }
  };
      ...
      >
        <View style={styles.headerContainer}>
          <View style={styles.infoContainer}>
            <View style={styles.avatarContainer}>
              {user?.avatar_url ? (
                <Avatar.Image size={80} source={{ uri: user.avatar_url }} style={styles.avatar} />
              ) : (
            ...
            <View style={styles.userInfoContainer}>
              <View style={styles.userContainer}>
                <Text style={styles.userInfo}>{user?.name || 'Unknown'}, {age}</Text>
                <Icon source="shield-check" color={user?.is_verified ? "#519CFF" : "#949494"} size={20} />
              </View>
              <View style={styles.userContainer}>
                <Icon source="map-marker" color="#949494" size={20} />
                <Text style={styles.userLocation}>{location || 'Unknown'}</Text>
              </View>
            </View>
          </View>
        </View>
        ...
        <View style={styles.gridContainer}>
          {validImages.length > 0 ? (
            ...
            <Text style={{ color: '#999' }}>No photos available</Text>
          )}
        </View>
